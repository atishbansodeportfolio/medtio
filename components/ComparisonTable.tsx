...
export default function ComparisonTable({ clinics }: { clinics: any[] }) {
  ...
  const SortIcon = ({ field }: { field: string }) => {
    ...
    return <span className="text-teal-600  ml-1 font-bold">{sortOrder === 'asc' ? '↑' : '↓'}</span>;
  };

  return (
    <div className="bg-white  rounded-xl shadow-sm border border-gray-200  overflow-hidden overflow-x-auto mb-16">
      <table className="w-full text-left border-collapse min-w-[800px]">
        <thead>
          <tr className="bg-gray-50  border-b border-gray-200  text-gray-700 ">
            ...
            <th 
              className="p-4 font-semibold text-center cursor-pointer hover:bg-gray-100 :bg-gray-800 transition-colors"
        ...
        <tbody className="divide-y divide-gray-200 ">
          {sortedClinics?.map((clinic: any, index: number) => (
              <tr key={clinic.id} className="transition-colors hover:bg-gray-50 :bg-gray-800/80">
                <td className="p-4 text-center">
                  <span className="font-bold text-gray-500 ">{index + 1}</span>
                </td>
                <td className="p-4">
                  <div className="flex flex-col items-start justify-center">
                    <Link href={`/${clinic.specialty_slug || 'ivf'}-clinics/${clinic.slug || clinic.id}`} className="text-teal-600  font-bold hover:underline">
                      {clinic.name}
                    </Link>
                  </div>
                </td>
                <td className="p-4">
                  <div className="flex items-center justify-center">
                    <span className="font-bold text-gray-900  mr-1">{clinic.rating || 'N/A'}</span>
                ...
                <td className="p-4 text-center">
                  <Link href={`/${clinic.specialty_slug || 'ivf'}-clinics/${clinic.slug || clinic.id}`} className="inline-block bg-teal-50  text-teal-600  hover:bg-teal-100 :bg-teal-900/50 font-semibold px-4 py-1.5 rounded-lg text-sm transition-colors border border-teal-100 ">
                    View
                  </Link>
                </td>
              </tr>
